...
public class DelegatingCommandExecutor : ICommandExecutor
{
    ...
    private static FlowT<CommandState, CommandOutcome> ExecuteExecutorIO(ICommand cmd, ICommandExecutor executor) =>
    (
    ...
    public async Task<Either<FlowError, CommandOutcome>> ExecuteAsync(ICommand cmd, CancellationToken cancelToken = default)
    {
        ...
        var outcome = await _transformerStack.Invoke(initialState, envIO);
