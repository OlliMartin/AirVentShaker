  ...
  Monad<Flow<TFlowConfiguration>>
{
  ...
  public static K<Flow<TFlowConfiguration>, A> LiftIO<A>(IO<A> ma) => new FlowT<TFlowConfiguration, A>(
    MonadIO
      .liftIO<ReaderT<TFlowConfiguration, EitherT<FlowError, IO>>, A>(ma).As()
  );

  /* Transformer Stack */
  public static FlowT<TFlowConfiguration, A> lift<A>(Either<FlowError, A> ma) => new(
    ReaderT.lift<TFlowConfiguration, EitherT<FlowError, IO>, A>(EitherT<FlowError, IO>.lift(ma))
  );

  public static FlowT<TFlowConfiguration, A> liftIO<A>(IO<A> ma) => new(
